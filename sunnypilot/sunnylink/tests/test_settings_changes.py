...
class TestLaneFitSourceMode:
  def test_lane_fit_source_mode_options(self, schema):
    item = _find_item(schema, "LaneFitSourceMode")
    assert item is not None, "LaneFitSourceMode item must be present"
    assert [opt["value"] for opt in item["options"]] == ["off", "shadow", "apply"]

  def test_lane_fit_source_mode_gated_by_custom_lateral_demand(self, schema):
    item = _find_item(schema, "LaneFitSourceMode")
    assert item is not None
    enablement = item.get("enablement") or []
    assert any(r.get("type") == "param" and r.get("key") == "CustomLateralDemandEnabled" for r in enablement)

  def test_lane_fit_source_mode_copy_mentions_monitor_only_and_requested_path(self, schema):
    item = _find_item(schema, "LaneFitSourceMode")
    assert item is not None
    description = item.get("description", "").lower()
    assert "monitor only" in description
...
class TestLaneCenteringOneLineMode:
  def test_lane_centering_one_line_mode_options(self, schema):
    item = _find_item(schema, "LaneCenteringOneLineMode")
    assert item is not None, "LaneCenteringOneLineMode item must be present"
    assert [opt["value"] for opt in item["options"]] == ["off", "shadow", "apply"]

  def test_lane_centering_one_line_mode_gated_by_custom_lateral_demand(self, schema):
    item = _find_item(schema, "LaneCenteringOneLineMode")
    assert item is not None
    enablement = item.get("enablement") or []
    assert any(r.get("type") == "param" and r.get("key") == "CustomLateralDemandEnabled" for r in enablement)

  def test_lane_centering_one_line_mode_copy_mentions_monitor_only_and_lane_width(self, schema):
    item = _find_item(schema, "LaneCenteringOneLineMode")
    assert item is not None
    description = item.get("description", "").lower()
    assert "monitor only" in description
    assert "lane width" in description
