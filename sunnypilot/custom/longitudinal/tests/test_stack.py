...
# --- creep calm: bounded-jerk final target through the low-speed close-lead regime ---

def _creep_inp(v_ego=0.8, d_rel=7.0, v_rel=0.1, **kw):
  return base(v_ego=v_ego, long_active=True, leads=(lead(d_rel=d_rel, v_lead=v_ego + v_rel, v_rel=v_rel), None), **kw)


def _decision(a=0.0, should_stop=False):
  return Decision(a_target=a, should_stop=should_stop, selected_intent="cruise", reason="baseline")


def test_creep_calm_collapses_intent_churn():
  # Route 290 shape: the arbitration output square-waves between -0.4 and +0.35 with
  # owner switches every ~0.15 s (13-14 per 2 s).
  s = CustomLongitudinalStack()
  inp = _creep_inp()
  raw = [(-0.40 if (i // 3) % 2 else 0.35) for i in range(160)]  # 8 s of churn
  outs = []
  for r in raw:
    out, dbg = s._apply_creep_calm(r, DT, inp, _decision(r), inp.leads[0])
    outs.append(out)
  deltas = [abs(b - a) for a, b in zip(outs, outs[1:])]
  # per-frame movement bounded by the creep jerk budget; symmetric slew = no drift to a
  # rail, so the settled band collapses far below the raw 0.75 p2p
  assert max(deltas) <= 0.9 * DT + 1e-9
  assert max(outs[-40:]) - min(outs[-40:]) < 0.2
  assert dbg["creep_calm_active"] is True


def test_creep_calm_hazard_and_regime_bypass():
  s = CustomLongitudinalStack()
  # prime state in-regime
  inp = _creep_inp()
  s._apply_creep_calm(0.2, DT, inp, _decision(0.2), inp.leads[0])
  # committed stop: full authority, state reset
  out, dbg = s._apply_creep_calm(-1.8, DT, inp, _decision(-1.8, should_stop=True), inp.leads[0])
  assert out == pytest.approx(-1.8)
  assert dbg["creep_calm_reason"] == "hazard_full_authority"
  # lead inside the proximity floor: full authority
  close = _creep_inp(d_rel=3.0)
  out, dbg = s._apply_creep_calm(-1.2, DT, close, _decision(-1.2), close.leads[0])
  assert out == pytest.approx(-1.2)
  # low TTC while closing: full authority
  ttc = _creep_inp(d_rel=5.0, v_rel=-2.5)
  out, dbg = s._apply_creep_calm(-1.0, DT, ttc, _decision(-1.0), ttc.leads[0])
  assert out == pytest.approx(-1.0)
  # out of regime (normal driving speed): untouched and inactive
  fast = base(v_ego=10.0, long_active=True, leads=(lead(d_rel=7.0), None))
  out, dbg = s._apply_creep_calm(-0.4, DT, fast, _decision(-0.4), fast.leads[0])
  assert out == pytest.approx(-0.4)
  assert dbg["creep_calm_active"] is False


def test_creep_calm_sustained_demand_still_arrives():
  # A persistent braking demand (not churn) reaches the target through the slew in ~1 s.
  s = CustomLongitudinalStack()
  inp = _creep_inp()
  out = 0.2
  s._apply_creep_calm(0.2, DT, inp, _decision(0.2), inp.leads[0])
  for _ in range(24):  # 1.2 s at DOWN_JERK 1.0
    out, _dbg = s._apply_creep_calm(-0.8, DT, inp, _decision(-0.8), inp.leads[0])
  assert out == pytest.approx(-0.8, abs=0.05)
