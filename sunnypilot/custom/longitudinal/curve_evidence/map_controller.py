      ...
      if d < MAX_SHORT_DROP_DISTANCE and (self.v_ego - tv) > MAX_SHORT_DROP_DELTA_V and tv < 0.7 * self.v_ego:
        ...
        for j in range(i + 1, len(forward_points)):
          ...
          if next_d >= MAX_SHORT_DROP_DISTANCE or next_tv > self.v_ego or next_tv >= 0.7 * self.v_ego:
            break
          confirm_count += 1
        already_target = (self.v_target > 0 and self.target_lat == target_velocity["latitude"] and
                          self.target_lon == target_velocity["longitude"])
        if confirm_count < SHORT_DROP_CONFIRM_POINTS and not already_target:
          continue
