...
J_EGO_COST = 5.0
A_CHANGE_COST = 145.0
SUNNYPILOT_CURRENT_A_CHANGE_COST = 200.0
...
COMFORT_BRAKE = 2.5
STOP_DISTANCE = 5.0
SUNNYPILOT_CURRENT_STOP_DISTANCE = 6.0
...
LEAD_APPROACH_RUNWAY_URGENT_DECEL = 2.0
LEAD_APPROACH_RUNWAY_COAST_DECEL = 0.6
LEAD_APPROACH_RUNWAY_MIN = 2.0
LEAD_APPROACH_RUNWAY_FULL_MIN = 5.0
LEAD_APPROACH_RUNWAY_MIN_SPAN = 1.0
...
LEAD_STOP_APPROACH_URGENCY_COST_FLOOR = 0.25
...
LEAD_TRANSITION_CLOSE_CURVE_HOLD_TIME_GAP = 2.2
...
def get_sunnypilot_current_jerk_factor(personality=log.LongitudinalPersonality.standard):
  if personality == log.LongitudinalPersonality.relaxed:
    return 1.0
  elif personality == log.LongitudinalPersonality.standard:
    return 1.0
...
def get_sunnypilot_current_T_FOLLOW(personality=log.LongitudinalPersonality.standard):
  if personality == log.LongitudinalPersonality.relaxed:
    return 1.75
  elif personality == log.LongitudinalPersonality.standard:
    return 1.45
...
def get_sunnypilot_current_safe_obstacle_distance(v_ego, t_follow):
  return (v_ego**2) / (2 * COMFORT_BRAKE) + t_follow * v_ego + SUNNYPILOT_CURRENT_STOP_DISTANCE
...
def should_hold_lead_transition_for_close_curve_lead(v_ego, d_rel, v_lead, y_rel, model_prob):
  closing_speed = v_ego - v_lead
  if abs(y_rel) >= LEAD_TRANSITION_CLOSE_CURVE_HOLD_MAX_Y_REL:
    return False
  if model_prob < LEAD_TRANSITION_THREAT_MODEL_PROB_MIN or closing_speed < LEAD_TRANSITION_CLOSE_CURVE_HOLD_CLOSING_MIN:
    return False
...
def get_dynamic_lead_approach_runway_blend(runway, closing_speed):
  runway = np.asarray(runway, dtype=float)
  closing_speed = np.maximum(np.asarray(closing_speed, dtype=float), 0.0)
  close_runway = np.maximum(
    LEAD_APPROACH_RUNWAY_MIN,
    closing_speed**2 / (2.0 * LEAD_APPROACH_RUNWAY_URGENT_DECEL),
  )
  full_runway = np.maximum(
    LEAD_APPROACH_RUNWAY_FULL_MIN,
    closing_speed**2 / (2.0 * LEAD_APPROACH_RUNWAY_COAST_DECEL),
  )
  full_runway = np.maximum(full_runway, close_runway + LEAD_APPROACH_RUNWAY_MIN_SPAN)
  return np.clip((runway - close_runway) / (full_runway - close_runway), 0.0, 1.0)
...
def get_progressive_lead_approach_gap(x_lead, v_ego, v_lead, t_follow, a_lead=0.0):
  ...
  late_ramp_blend = 1.0 - get_dynamic_lead_approach_runway_blend(runway_margin, closing_speed)
...
def get_lead_stop_approach_comfort_target(x_lead, v_ego, v_lead, a_lead, t_follow):
  ...
  urgency_blend = 1.0 - get_lead_stop_runway_urgency(x_lead, v_ego, v_lead, t_follow, a_lead)
  urgency_blend = np.maximum(
    urgency_blend,
    LEAD_STOP_APPROACH_URGENCY_COST_FLOOR * speed_blend * stopped_blend * decel_blend,
  )
...
def get_moving_lead_stop_approach_comfort_target(x_lead, v_ego, v_lead, a_lead, t_follow):
  ...
  danger_blend = np.maximum.reduce([danger_blend, danger_breach_blend, danger_floor_blend])
  ...
  near_desired_recovery_blend *= 1.0 - np.interp(
    closing_speed,
    MOVING_LEAD_STOP_APPROACH_COAST_RECOVERY_CLOSING_BP,
    [0.0, 1.0],
  )
...
def get_approach_follow_distance(x_lead, v_ego, v_lead, t_follow, a_lead=0.0):
  ...
  decel_blend *= 1.0 - get_dynamic_lead_approach_runway_blend(
...
def get_approach_runway_blend(x_lead, v_ego, v_lead, t_follow, a_lead=0.0):
  ...
  closing_speed = np.maximum(v_ego - v_lead, 0.0)
  return np.maximum(get_dynamic_lead_approach_runway_blend(runway, closing_speed), get_lead_stop_runway_preference(x_lead, v_ego, v_lead, t_follow, a_lead))
...
class LongitudinalMpc:
  ...
  def update(self, radarstate, v_cruise, personality=log.LongitudinalPersonality.standard, block_short_gap_pullaway_response=False,
    ...
    lead_0_hard_relaxation = get_progressive_lead_hard_obstacle_relaxation(
      lead_brake_xv_0[:, 0], v_ego, lead_brake_xv_0[:, 1], lead_0_brake_a_traj, t_follow, target_gap=lead_0_desired_gap,
    )
    lead_1_hard_relaxation = get_progressive_lead_hard_obstacle_relaxation(
      lead_brake_xv_1[:, 0], v_ego, lead_brake_xv_1[:, 1], lead_1_brake_a_traj, t_follow, target_gap=lead_1_desired_gap,
    )
    lead_0_obstacle = (
...
class SunnypilotLongitudinalMpc(LongitudinalMpc):
  def set_weights(self, prev_accel_constraint=True, personality=log.LongitudinalPersonality.standard):
    set_weights_key = (bool(prev_accel_constraint), str(personality), "sunnypilot-current")
    if self._last_set_weights_key == set_weights_key:
      return

    jerk_factor = get_sunnypilot_current_jerk_factor(personality)
    a_change_cost = SUNNYPILOT_CURRENT_A_CHANGE_COST if prev_accel_constraint else 0
    self.cost_weights = [X_EGO_OBSTACLE_COST, X_EGO_COST, V_EGO_COST, A_EGO_COST, jerk_factor * a_change_cost, jerk_factor * J_EGO_COST]
    self.constraint_cost_weights = [LIMIT_COST, LIMIT_COST, LIMIT_COST, DANGER_ZONE_COST]
    self._last_set_weights_key = set_weights_key
    self.set_cost_weights(self.cost_weights, self.constraint_cost_weights)

  def process_sunnypilot_current_lead(self, lead):
    v_ego = self.x0[1]
    if lead is not None and lead.status:
      x_lead = float(lead.dRel)
      v_lead = float(lead.vLead)
    ...
    v_lead = np.clip(v_lead, 0.0, 1e8)
    a_lead = np.clip(a_lead, -10.0, 5.0)
    lead_xv, _ = self.extrapolate_lead(x_lead, v_lead, a_lead, a_lead_tau)
    return lead_xv

  def update(self, radarstate, v_cruise, personality=log.LongitudinalPersonality.standard, **_kwargs):
    t_follow = get_sunnypilot_current_T_FOLLOW(personality)
    v_ego = self.x0[1]
    self.status = radarstate.leadOne.status or radarstate.leadTwo.status
    ...
    lead_xv_0 = self.process_sunnypilot_current_lead(radarstate.leadOne)
    lead_xv_1 = self.process_sunnypilot_current_lead(radarstate.leadTwo)
    ...
    self.run()
    lead_one_model_prob = float(getattr(radarstate.leadOne, "modelProb", 1.0)) if radarstate.leadOne.status else 0.0
    if np.any(lead_xv_0[FCW_IDXS, 0] - self.x_sol[FCW_IDXS, 0] < CRASH_DISTANCE) and lead_one_model_prob > 0.9:
      self.crash_cnt += 1
    else:
      self.crash_cnt = 0
