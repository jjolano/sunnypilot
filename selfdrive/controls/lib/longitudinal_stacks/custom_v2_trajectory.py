...
def preserve_seed_trajectory(output: LongitudinalStackOutput, a_target: float) -> bool:
  if bool(output.debug.get("planner_seed_scalar", False)):
    return False
  return math.isclose(float(output.a_target), float(a_target), abs_tol=A_TARGET_EPS)


def synth_custom_v2_trajectory(output: LongitudinalStackOutput, v_ego: float,
