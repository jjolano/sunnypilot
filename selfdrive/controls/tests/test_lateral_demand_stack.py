...
def test_resolve_lateral_demand_stack_returns_expected_class():
  assert isinstance(build_lateral_demand_stack(resolve_lateral_demand_stack("sunnypilot-current")), SunnypilotCurrentLateralDemandStack)
  assert isinstance(build_lateral_demand_stack(resolve_lateral_demand_stack("custom-2.0")), CustomV2LateralDemandStack)
  assert isinstance(build_lateral_demand_stack(resolve_lateral_demand_stack("custom-experimental")), CustomExperimentalLateralDemandStack)
...
def test_lateral_demand_stack_update_returns_stack_output_with_legacy_and_profile():
  stack = build_lateral_demand_stack(resolve_lateral_demand_stack("sunnypilot-current"))
  demand = _make_demand()
...
def test_custom_recommended_profile_maps_to_custom_recommended_and_torque_4_1():
  mapping = controls_profile_mapping_for(ControlsProfileId.CUSTOM_RECOMMENDED)
  assert mapping.lateral_demand_stack == LateralDemandStackId.CUSTOM_RECOMMENDED
  assert mapping.torque_control_tune.value == "4.1"
