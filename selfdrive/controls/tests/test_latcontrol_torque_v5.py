...
from openpilot.sunnypilot.selfdrive.controls.lib.latcontrol_torque_v5 import LatControlTorque
...
def test_v5_logging_fields_are_populated():
...
def test_v5_release_on_override():
  controller, VM = get_controller(TOYOTA.TOYOTA_COROLLA_TSS2)
...
def test_v5_reports_learning_frozen_at_low_speed():
  controller, VM = get_controller(TOYOTA.TOYOTA_COROLLA_TSS2)
...
def test_v5_blends_low_speed_reference_transitions():
  controller, VM = get_controller(TOYOTA.TOYOTA_COROLLA_TSS2)
  ...
  assert 0.05 < lac_log.desiredLateralAccel < 0.5


def test_v5_low_speed_same_sign_unwind_trims_residual():
  controller, VM = get_controller(TOYOTA.TOYOTA_COROLLA_TSS2)
  ...
  assert lac_log.output < adaptive_log.nominalOutput
