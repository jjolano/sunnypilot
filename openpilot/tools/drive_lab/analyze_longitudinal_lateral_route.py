...
def extract_rows(msgs: list[Any], window: AnalysisWindow) -> list[dict[str, Any]]:
  ...
  for msg in ordered:
    ...
    if _is_relevant_message(typ):
      ...
      if typ == "controlsState" and window.start_s <= t <= window.end_s:
        row = dict(state)
...
def update_state(state: dict[str, Any], typ: str, payload: Any) -> None:
  if typ == "selfdriveState":
    ...
    _update_model_path_state(state, _safe_get(payload, "modelPathState"), _safe_get(payload, "desiredCurvature"))
...
def _update_model_path_state(state: dict[str, Any], path_state: Any, processed_curvature: Any) -> None:
  for column, path in (
  ...
  _set(state, "model_path_processed_curvature", processed_curvature)
