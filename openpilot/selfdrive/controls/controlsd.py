...
class Controls(ControlsExt):
  ...
  def state_control(self):
    ...
    maneuver_active = self.sm.valid['lateralManeuverPlan']
    if maneuver_active:
      new_desired_curvature = self.sm['lateralManeuverPlan'].desiredCurvature if CC.latActive else self.curvature
    else:
      new_desired_curvature = model_v2.action.desiredCurvature if CC.latActive else self.curvature
    self.raw_desired_curvature = new_desired_curvature
    # Fork-owned custom lateral conditioning (fail-closed; returns the raw curvature when off).
    new_desired_curvature = self.conditioned_lateral_demand(
      self.sm, CC.latActive, CS, lp.roll, new_desired_curvature, self.curvature,
      self.calibrated_pose, self.steer_limited_by_safety, self.curvature_limited,
      lat_delay, maneuver_active)
    # Generic hard curvature/lat-accel caps stay here: Conditioned -> Processed Lateral Demand.
