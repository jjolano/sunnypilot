...
def test_build_lateral_profile_ranks_straight_oscillation_windows(tmp_path):
  ...
  for i in range(80):
    ...
    msgs.extend([
      msg("carState", t, vEgo=15.0, steeringPressed=False, leftBlinker=False, rightBlinker=False, steeringAngleDeg=steering_angle),
      msg("carControl", t, latActive=True),
      msg("carOutput", t, actuatorsOutput=SimpleNamespace(torque=steering_angle / 20.0)),
      msg("modelV2", t, meta=SimpleNamespace(laneChangeState=FakeEnum("off"))),
      controls_msg(t, raw_curvature=raw, steering_output=steering_angle / 20.0),
    ])
